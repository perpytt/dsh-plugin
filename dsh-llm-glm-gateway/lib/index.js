...
* @module dsh-llm-glm-gateway/serialize
*/
...
function reasoningEffort(effort) {
	...
	throw new LlmError(`GLM gateway does not support reasoning effort "${effort}"`, "UNSUPPORTED_REASONING_EFFORT");
}
...
function resolveThinking(options, defaults) {
	...
	if (defaults.thinking === "disabled" && effort !== void 0 && effort !== "off") throw new LlmError(`GLM gateway deployment does not support reasoning effort "${effort}"`, "UNSUPPORTED_REASONING_EFFORT");
...
/** DeepSeek Files API identifiers. @module dsh-llm-glm-gateway/file-id */
/**
...
/** OpenAI-compatible DeepSeek Files API transport. @module dsh-llm-glm-gateway/files-api */
...
/** Durable DeepSeek attachment-to-file-id index. @module dsh-llm-glm-gateway/upload-index */
...
function parseIndex(text) {
	...
	try {
		...
		throw new InvalidUploadIndexError("llm-glm-gateway: upload index is not valid JSON", { cause: error });
	}
	if (value === null || typeof value !== "object" || Array.isArray(value)) throw new InvalidUploadIndexError("llm-glm-gateway: upload index is not an object");
	const index = value;
	if (index.formatVersion !== 3 || !Array.isArray(index.records)) throw new InvalidUploadIndexError("llm-glm-gateway: unsupported upload index format");
	const records = index.records.map(parseRecord);
	const keys = /* @__PURE__ */ new Set();
	for (const record of records) {
		const key = `${record.scope}\0${record.variantId}`;
		if (keys.has(key)) throw new InvalidUploadIndexError("llm-glm-gateway: upload index contains duplicate mappings");
		keys.add(key);
	}
	return {
		formatVersion: 3,
...
var DeepSeekUploadIndex = class {
	...
	path;
	/**
	* @param path - explicit test path; omission uses `DSH_HOME/llm-glm-gateway/files-v3.json`.
	*/
	constructor(path = join(resolveDshHome(), "llm-glm-gateway", "files-v3.json")) {
		this.path = path;
	}
	async load() {
		try {
...
/** DeepSeek Files API upload reuse, invalidation, and quota recovery. @module dsh-llm-glm-gateway/file-store */
...
* @module dsh-llm-glm-gateway/translate
*/
/**
...
/**
* Strip an empty-object prefix that the upstream gateway sometimes prepends
* to a tool-call `arguments` string (observed as `{}{"city":"北京"}` on this
* route's non-stream path). Each pass removes one literal `{}` when what
* follows still opens a JSON object, so `{}{}{...}` is handled too while a
* genuine single object — including one whose values contain nested
* objects — is left untouched.
* @param raw - concatenated wire `arguments` text.
* @returns a JSON-object string without any leading empty-object prefix.
*/
function sanitizeToolArguments(raw) {
	if (typeof raw !== "string") return raw;
	let text = raw;
	while (text.startsWith("{}") && text.length > 2) {
...
function closeBlock(block) {
	switch (block.kind) {
		...
		case "tool-call": return {
			...
			name: block.name ?? "",
			arguments: sanitizeToolArguments(block.text)
		};
	}
}
/**
...
*
* @module dsh-llm-glm-gateway/adapter
*/
...
/** Default per-request output-token cap (this gateway validates max_tokens within [1, 131072]). */
...
/**
* Register a {@link DeepSeekAdapter} for the `glm-gateway` provider route on
* `ctx.llm`, with connection facts resolved per request instead of frozen at
* load: the plugin layers its `cordis.yml` entry config under the optional
* `llm-glm-gateway` user-settings section (`ctx.settings`) and resolves the API
* key through the optional credential seam (`ctx.credentials`), so a changed
* base URL, catalog, or key reaches the very next request without restarting
* anything, while an in-flight stream keeps the facts it started with. The
* one registration-captured fact — the retry policy — re-registers the route
* in place when it changes.
* @module @deepseek-ai/dsh-llm-glm-gateway
*/
const name = "llm-glm-gateway";
const inject = ["llm"];
const NS = settingsNamespace("llm-glm-gateway");
const DEFAULT_API_KEY_ENV = "TEST_KEY";
/** The single provider route this plugin owns. */
const PROVIDER = "glm-gateway";
...
const BASE_URL_ENV = "GLM_GATEWAY_BASE_URL";
/** Resolve, validate, and detach the advisory model catalog. */
function resolveModels(models) {
	const seen = /* @__PURE__ */ new Set();
	return (models ?? DEFAULT_MODELS).map((model) => {
		if (model.id.length === 0) throw new Error("llm-glm-gateway: catalog model ids must be non-empty");
		if (model.name !== void 0 && model.name.length === 0) throw new Error(`llm-glm-gateway: catalog model "${model.id}" has an empty name`);
		if (model.contextWindow !== void 0 && (!Number.isInteger(model.contextWindow) || model.contextWindow <= 0)) throw new Error(`llm-glm-gateway: catalog model "${model.id}" contextWindow must be a positive integer`);
		if (model.maxTokens !== void 0 && (!Number.isInteger(model.maxTokens) || model.maxTokens <= 0)) throw new Error(`llm-glm-gateway: catalog model "${model.id}" maxTokens must be a positive integer`);
		const inputModalities = model.inputModalities ?? ["text"];
		if (inputModalities.length === 0) throw new Error(`llm-glm-gateway: catalog model "${model.id}" inputModalities must not be empty`);
		if (inputModalities.some((modality) => !MODEL_MODALITIES.includes(modality))) throw new Error(`llm-glm-gateway: catalog model "${model.id}" inputModalities must contain only "text" and "image"`);
		if (new Set(inputModalities).size !== inputModalities.length) throw new Error(`llm-glm-gateway: catalog model "${model.id}" inputModalities must not contain duplicates`);
		const hasImage = inputModalities.includes("image");
		if (!hasImage && (model.imagePixelBudget !== void 0 || model.imageMaxBytes !== void 0 || model.imageDetail !== void 0)) throw new Error(`llm-glm-gateway: text-only catalog model "${model.id}" cannot declare image request limits`);
		if (model.imagePixelBudget !== void 0 && (!Number.isSafeInteger(model.imagePixelBudget) || model.imagePixelBudget <= 0)) throw new Error(`llm-glm-gateway: catalog model "${model.id}" imagePixelBudget must be a positive safe integer`);
		if (model.imageMaxBytes !== void 0 && (!Number.isSafeInteger(model.imageMaxBytes) || model.imageMaxBytes <= 0)) throw new Error(`llm-glm-gateway: catalog model "${model.id}" imageMaxBytes must be a positive safe integer`);
		if (seen.has(model.id)) throw new Error(`llm-glm-gateway: duplicate catalog model "${model.id}"`);
		seen.add(model.id);
		return {
			id: model.id,
...
function resolveAdapterOptions(config, environment) {
	if (config.thinking === "disabled" && config.reasoningEffort !== void 0 && config.reasoningEffort !== "off") throw new Error("llm-glm-gateway: only reasoningEffort \"off\" can be configured when thinking is disabled");
	if (config.defaultContextWindow !== void 0 && (!Number.isInteger(config.defaultContextWindow) || config.defaultContextWindow <= 0)) throw new Error("llm-glm-gateway: defaultContextWindow must be a positive integer");
	if (config.maxTokens !== void 0 && (!Number.isSafeInteger(config.maxTokens) || config.maxTokens <= 0)) throw new Error("llm-glm-gateway: maxTokens must be a positive safe integer");
	const streamIdleTimeoutMs = config.streamIdleTimeoutMs ?? 3e5;
	if (!Number.isFinite(streamIdleTimeoutMs) || streamIdleTimeoutMs <= 0 || streamIdleTimeoutMs > MAX_TIMER_DELAY_MS) throw new Error(`llm-glm-gateway: streamIdleTimeoutMs must be a positive finite number no greater than ${MAX_TIMER_DELAY_MS}`);
	const maxRequestFilesBytes = config.maxRequestFilesBytes ?? 134217728;
	if (!Number.isSafeInteger(maxRequestFilesBytes) || maxRequestFilesBytes <= 0) throw new Error("llm-glm-gateway: maxRequestFilesBytes must be a positive safe integer");
	const maxInlineRequestImageBytes = config.maxInlineRequestImageBytes ?? 20971520;
	if (!Number.isSafeInteger(maxInlineRequestImageBytes) || maxInlineRequestImageBytes <= 0) throw new Error("llm-glm-gateway: maxInlineRequestImageBytes must be a positive safe integer");
	const maxImagesPerRequest = config.maxImagesPerRequest ?? 600;
	if (!Number.isSafeInteger(maxImagesPerRequest) || maxImagesPerRequest <= 0) throw new Error("llm-glm-gateway: maxImagesPerRequest must be a positive safe integer");
	const imageOffloadByteQuantum = config.imageOffloadByteQuantum ?? 67108864;
	if (!Number.isSafeInteger(imageOffloadByteQuantum) || imageOffloadByteQuantum <= 0) throw new Error("llm-glm-gateway: imageOffloadByteQuantum must be a positive safe integer");
	if (imageOffloadByteQuantum > maxRequestFilesBytes) throw new Error("llm-glm-gateway: imageOffloadByteQuantum must not exceed maxRequestFilesBytes");
	const inlineImageOffloadByteQuantum = config.inlineImageOffloadByteQuantum ?? 10485760;
	if (!Number.isSafeInteger(inlineImageOffloadByteQuantum) || inlineImageOffloadByteQuantum <= 0) throw new Error("llm-glm-gateway: inlineImageOffloadByteQuantum must be a positive safe integer");
	if (inlineImageOffloadByteQuantum > maxInlineRequestImageBytes) throw new Error("llm-glm-gateway: inlineImageOffloadByteQuantum must not exceed maxInlineRequestImageBytes");
	const imageOffloadCountQuantum = config.imageOffloadCountQuantum ?? 20;
	if (!Number.isSafeInteger(imageOffloadCountQuantum) || imageOffloadCountQuantum <= 0) throw new Error("llm-glm-gateway: imageOffloadCountQuantum must be a positive safe integer");
	if (imageOffloadCountQuantum > maxImagesPerRequest) throw new Error("llm-glm-gateway: imageOffloadCountQuantum must not exceed maxImagesPerRequest");
	const filesApiTimeoutMs = config.filesApiTimeoutMs ?? 6e4;
	if (!Number.isFinite(filesApiTimeoutMs) || filesApiTimeoutMs <= 0 || filesApiTimeoutMs > MAX_TIMER_DELAY_MS) throw new Error(`llm-glm-gateway: filesApiTimeoutMs must be a positive finite number no greater than ${MAX_TIMER_DELAY_MS}`);
	const fileExpiresAfterSeconds = config.fileExpiresAfterSeconds ?? 604800;
	if (!Number.isSafeInteger(fileExpiresAfterSeconds) || fileExpiresAfterSeconds < 3600 || fileExpiresAfterSeconds > 2592e3) throw new Error("llm-glm-gateway: fileExpiresAfterSeconds must be an integer from 3600 through 2592000");
	const fileRefreshMarginSeconds = config.fileRefreshMarginSeconds ?? 3600;
	if (!Number.isSafeInteger(fileRefreshMarginSeconds) || fileRefreshMarginSeconds < 0 || fileRefreshMarginSeconds >= fileExpiresAfterSeconds) throw new Error("llm-glm-gateway: fileRefreshMarginSeconds must be a non-negative integer below fileExpiresAfterSeconds");
	const fileQuotaCleanupBatch = config.fileQuotaCleanupBatch ?? 100;
	if (!Number.isSafeInteger(fileQuotaCleanupBatch) || fileQuotaCleanupBatch < 1 || fileQuotaCleanupBatch > 1e3) throw new Error("llm-glm-gateway: fileQuotaCleanupBatch must be an integer from 1 through 1000");
	return {
		...
		retryPolicy: resolveRetryPolicy(config.retryPolicy, "llm-glm-gateway: retryPolicy")
	};
}
function apply(ctx, config) {
	...
	const options = () => {
		...
		try {
			...
			lastRaw = raw;
			ctx.logger.error("llm-glm-gateway: keeping the last good configuration after an invalid settings section");
	...
	const resolveApiKey = async (connection) => {
		...
		if (credentials !== void 0) {
			...
			if (hit !== void 0) return assertUsableApiKey(hit.value, "llm-glm-gateway", ref);
		} else {
			const ambient = launchEnvironmentOf(ctx).get(ref);
			if (ambient !== void 0 && ambient.value.length > 0) return assertUsableApiKey(ambient.value, "llm-glm-gateway", ref);
		}
		throw new LlmError(`llm-glm-gateway: no API key for provider route "${PROVIDER}"; store ${ref} through the credentials service (the web Models page writes it), or export ${ref} in the launching environment`, "MISSING_CREDENTIAL");
	};
	let userId;
